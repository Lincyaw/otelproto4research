...
def __parse_attributes(attributes: List[metrics_pb2.KeyValue]) -> Dict[str, Any]:
    return {attr.key: attr.value for attr in attributes}

def __parse_number_data_point(point: metrics_pb2.NumberDataPoint) -> Dict[str, Any]:
    data = {
        "start_time_unix_nano": point.start_time_unix_nano,
        "time_unix_nano": point.time_unix_nano,
        "attributes": parse_attributes(point.attributes),
        "value": point.as_double if point.HasField("as_double") else point.as_int,
        "flags": point.flags
    }
    return data

def __parse_histogram_data_point(point: metrics_pb2.HistogramDataPoint) -> Dict[str, Any]:
    data = {
        "start_time_unix_nano": point.start_time_unix_nano,
        "time_unix_nano": point.time_unix_nano,
        "attributes": parse_attributes(point.attributes),
...
def __parse_summary_data_point(point: metrics_pb2.SummaryDataPoint) -> Dict[str, Any]:
    data = {
        "start_time_unix_nano": point.start_time_unix_nano,
        "time_unix_nano": point.time_unix_nano,
        "attributes": parse_attributes(point.attributes),
        "count": point.count,
        "sum": point.sum,
        "quantile_values": {qv.quantile: qv.value for qv in point.quantile_values},
        "flags": point.flags
    }
    return data
            ...
            for m in scope_metric.metrics:
                ...
                if m.HasField("gauge"):
                    for point in m.gauge.data_points:
                        row = {**common_data, **parse_number_data_point(point)}
                        rows.append(row)
                elif m.HasField("sum"):
                    ...
                    for point in m.histogram.data_points:
                        row = {**common_data, **parse_histogram_data_point(point)}
                        rows.append(row)
                    ...
                    for point in m.summary.data_points:
                        row = {**common_data, **parse_summary_data_point(point)}
                        rows.append(row)
